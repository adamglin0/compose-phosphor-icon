...
public val BoldGroup.`Dice-one-bold`: ImageVector
    get() {
        if (`_dice-one-bold` != null) {
            return `_dice-one-bold`!!
        }
        `_dice-one-bold` = Builder(name = "Dice-one-bold", defaultWidth = 256.0.dp, defaultHeight =
...
private var `_dice-one-bold`: ImageVector? = null
